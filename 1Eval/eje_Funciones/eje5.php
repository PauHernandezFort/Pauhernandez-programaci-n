...
function edadhumana ($edadCan): float{
    $edadPerro = 31 +16*log($edadCan);
    return $edadPerro;
}
function sobrepeso($arrayCan):int{
    $panza = 0;
    foreach($arrayCan as $chuchu){
        if(edadhumana($chuchu["edad"]) >60 && ($chuchu["peso"] >7)){
        $panza ++;
        
    }  elseif(edadhumana($chuchu["edad"])< 40 && ($chuchu["peso"] >3)){
        $panza ++;
        }elseif($chuchu["peso"] <6){
            $panza ++;
        }
    }
...
echo "en total hay ", sobrepeso($data),"perritos con panza";
?>