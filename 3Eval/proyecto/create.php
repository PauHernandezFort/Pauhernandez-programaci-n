...
<body>
    <form name='FormularioDatos' method='post' action=''>
        ...
        <input type='text' id='pat' name='pat' value=''><br>
        <label for='dat'>Date</label><br>
        <input type='date' id='dat' name='dat' value=''><br>
        <label for='amo'>Amount</label><br>
        <input type='number' id='amo' name='amo'  value='' required><br>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($be == "yes"){
    $visits = $datos->getVisits();
    end($visits);
    $idL = current($visits);
    $id = $idL->getId() +1;
    $newItem = new Visit($id,$nPatient, $nAmount, $nDate, $nDefaulter);
