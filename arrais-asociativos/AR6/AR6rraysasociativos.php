...
$esur = 0;
$contador = 0;
echo "<h3>", "El listado estaciones han bajado de -20 grados o estan por encima de 40 grados.","</h3>", "<br>";
foreach ($data as $cordenadas){
    $separacion = explode(",", $cordenadas["coordenadas"]);
   if($separacion[0] > 0){
    $enorte ++;
   }
    else {
    $temperatura += $data[$contador]["numberrange"];
    $esur ++;
   } 
$contador++;    
}
for ($i=0; $i < $tamaño ; $i++) { 
    if($data[$i]["numberrange"] <= -20 || $data[$i]["numberrange"] >= 40){
    $estcion = $data[$i]["estacion"];
    echo $estcion, "<br>";
}
}
echo "<br>";
echo "<br>";
echo "$enorte Estaciones pertenecen al emisferio norte", "<br>";
echo $temperatura / $esur, " Es la temperatura media de el emisferio sur";
